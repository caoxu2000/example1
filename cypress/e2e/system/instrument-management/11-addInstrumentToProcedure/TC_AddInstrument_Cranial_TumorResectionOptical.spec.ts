...
describe('Requirement Number: 11-2, Juno Requirement ID: CR101415', () => {
  ...
  before(() => {
    ...
    util.exit()
    cy.visit('/')
    procedure.action.select(procedure.opt.tumorResectionOptical)
  })
  ...
  it(`TC_AddInstrument_Cranial_TumorResectionOptical: verify ${instruments.passivePlanarBlunt.name} can be added to procedure`, () => {
    instrument.task.action.clickInstrumentCategory(
      instruments.passivePlanarBlunt.category
    )
    ...
    instrument.task.action.addAnInstrumentToProcedure(
      instruments.passivePlanarBlunt
    )
    instrument.task.assert.isToolInProcedure(
      instruments.passivePlanarBlunt,
